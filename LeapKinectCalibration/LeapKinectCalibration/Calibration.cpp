...
void Calibration::InitParam(int LoopCount, float Threshold, int samplecount){
	m_N = LoopCount;
	m_Threshold = Threshold;
	m_m = samplecount;
}

void Calibration::DataStore(cv::Point3f first, cv::Point3f second){
...
void Calibration::SelectRandomNum(int *randBox){
...
int Calibration::CalcLoopNUM(float p, float alpha, int samplecount){
	return log(1.0f - p) / log(1.0f - pow(alpha, samplecount));
}

int Calibration::CalcInlierCount(cv::Mat RT, float threshold, float *averError){
	int iCount = 0;
	*averError = 0.0f;
...
float Calibration::CalcDist(cv::Mat RT, int idx){
	// M * X1 = X2
...
// 'X1' = M*X2
void Calibration::CreateRefMat(int *idxarr, cv::Mat* Mat1){
...
// X1 = M * 'X2'
void Calibration::CreateTargetMat(int *idxarr, cv::Mat* Mat2, int idx){
	cv::Point3f firstPoint, secondPoint;
...
void Calibration::printMat(cv::Mat src){
...
cv::Mat Calibration::GetRTMatrix(){
	return RTMat;
}