	...
	if(m_pDepthRGBX && pBuffer){
		...
		for(int i = 0; i < nWidth*nHeight; i++){
			...
			for(int j = 0; j < 4; j++){
				...
				src->data[4*i+j] = (byte)(depth >= nMinDepth && depth <= nMaxDepth ? (depth % 256) : 0);
			}
	...
	for( int i = 0; i < BODY_COUNT; i++){
		...
		if (bFaceTracked)
		{
			...
			if(faceBox.Right != 0 && faceBox.Left != 0){
				...
				m_SkeletonInfo->InfoBody[tbodyidx].Face.faceDir.x = faceRotation.x;
				m_SkeletonInfo->InfoBody[tbodyidx].Face.faceDir.y = faceRotation.y;
				m_SkeletonInfo->InfoBody[tbodyidx].Face.faceDir.z = faceRotation.z;
				m_SkeletonInfo->InfoBody[tbodyidx].Face.faceDir.w = faceRotation.w;
			}
...
HRESULT KinectConnector::MapColorToCameraCoordinates(const std::vector<ColorSpacePoint>& colorsps, std::vector<CameraSpacePoint>& camerasps)
{
	//Access frame
	if( !m_pDepthFrameReader){
		return E_PENDING;
	...
	if (SUCCEEDED(hr))
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			hr = m_pCoordinateMapper->MapColorFrameToCameraSpace(512 * 424, pBuffer, 1920 * 1080, cameraSpacePoints);
			if (SUCCEEDED(hr))
			{
